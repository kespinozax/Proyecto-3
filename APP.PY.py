...
# ---------------- Variable global para comparar algoritmos ----------------
resultados_globales = {}
...
def animar(procesos, algoritmo="FCFS", quantum=2, intervalo_ms=3000):
    ...
    if algoritmo == "FCFS":
        ...
        messagebox.showinfo("Info", f"Algoritmo {algoritmo} no implementado")
        return
    ...
    # ---------------- Tabla de Eficiencia ----------------
    for p in resultados:
        p.eficiencia = round(p.duracion / p.retorno, 2) if p.retorno > 0 else 0

    mas_eficiente = max(resultados, key=lambda x: x.eficiencia)
    ...
    data = {
        ...
        "Eficiencia": [p.eficiencia for p in resultados],
        "Estado": ["Más eficiente" if p == mas_eficiente else "" for p in resultados],
    }
    ...
    fila_destacada = resultados.index(mas_eficiente) + 1
    for j in range(len(df.columns)):
        tabla[(fila_destacada, j)].set_facecolor("#90EE90")

    ax3.set_title("Tabla de Eficiencia", pad=8)

    # Guardar eficiencia promedio para comparación
    ef_prom = sum(p.eficiencia for p in resultados) / len(resultados)
    resultados_globales[algoritmo] = ef_prom
    print(f"Eficiencia promedio {algoritmo}: {ef_prom:.2f}")
...
# ---------------- Comparación de eficiencia global ----------------
def mostrar_comparacion():
    if not resultados_globales:
        messagebox.showinfo("Info", "Primero ejecuta al menos un algoritmo.")
        return

    nombres = list(resultados_globales.keys())
    valores = list(resultados_globales.values())

    plt.figure(figsize=(6, 4))
    plt.bar(nombres, valores, color=["skyblue", "lightgreen", "salmon"])
    plt.title("Comparación de Eficiencia Promedio entre Algoritmos")
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("Simulador de Planificación de Procesos (FCFS, SJF, RR)")
        self.procesos = []
        ...
        self.comparar_btn = tk.Button(self.frame_algoritmos, text="Comparar Eficiencia", command=mostrar_comparacion)
        self.comparar_btn.pack(side=tk.LEFT, padx=5)
    ...
    def ejecutar(self, algoritmo):
        ...
        try:
            ...
            quantum = 2
        self.animacion = animar(list(self.procesos), algoritmo=algoritmo, quantum=quantum)
